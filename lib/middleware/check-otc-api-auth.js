...
var
	...
	fetchAuthMiddleware = require("./fetch-auth")
;
...
module.exports = function chechOtcApiAuth(req, res, next) {
	...
	if (authHeader) {
		...
		if (authPrefix === 'basic') {
			...
			logger.debug("Basic Credentials service_id: " + id + ", expected: " + nconf.get("TIAM_CLIENT_ID"));
			logger.debug("Basic Credentials broker_secret: " + secret + ", expected: " + nconf.get("OTC_API_BROKER_SECRET"));
			...
			return next();
		} else if (authPrefix === 'bearer') {
			// Temp for Step 3 adoption
			return fetchAuthMiddleware(req.tiamClient)(req, res, next);
		}
	}
	return res.status(401).json({ message: 'An invalid authorization header was passed in.' });
}
