...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, serviceCredentials, parametersData) {
	...
	if (api_token) {
		slackClient.getSlackUser(api_token, function (err, user) {
			...
			tiamUtil.setServiceData(serviceCredentials, {api_token: api_token}, function(err, result) {
				if (err) {
    ...
    db.get(serviceInstanceId, null, function(err, body) {
    	...
    	async.waterfall([
    	    function findData(callback) {
    	        if (patchParameters.api_token) {
    	        	callback(null, {api_token: patchParameters.api_token});
    	        } else {
					tiamUtil.getServiceData(body.service_credentials, callback);
    	        }
    	    },
    	    function (data, callback) {
    	    	...
    	    	slackClient.getSlackUser(api_token, function (err, user) {
					...
					tiamUtil.setServiceData(body.service_credentials, {api_token: api_token}, function(err, result) {
						if (err) {
							logger.error(logPrefix + "Error while storing API Key in TIAM Service:" + err);
							return res.status(500).json({ "description": "Error while storing API Key." });
						}
...
function bindServiceInstanceToToolchain (req, res/*, next*/) {
	...
	.update(function (err, doc) {
		...
		if (updatedDocument.channel_newly_created) {
			tiamUtil.getServiceData(updatedDocument.service_credentials, function(err, data) {
				if (err) {
					logger.error(logPrefix + "Error while getting API Key from TIAM Service.");
				} else {
					changeChannelTopicAndPurpose(data.api_token, updatedDocument.instance_id, toolchainId, toolchainCredentials);				
				}
			})
		}
		...
		// TODO Only for local-dev test
		if (false) {
			// Provide the notification url for the toolchain lifecycle event - in case of no LMS
