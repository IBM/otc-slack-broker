...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, parametersData) {
	...
	if (api_token) {
		slackUtils.getSlackUser(api_token, function (err, user) {
			...
			// Temporary workaround to have a topic and purpose
			if (!channel_topic) {
			...
			slackUtils.getOrCreateSlackChannel(api_token, user, channel_id, channel_name, channel_topic, channel_purpose, function (err, channel) {
				if (err) {
	...
	.update(function (err) {
		...
		if (true) {
			logger.debug(logPrefix + "Binding the service instance with" +
					" ID: " + serviceInstanceId + " to toolchain ID: " + toolchainId +
					" done");
			// Old version w/o toolchain_lifecycle_webhook_url
			return res.status(204).end();
		} else {
			// Provide the notification url for the toolchain lifecycle event
			var toolchain_lifecycle_webhook_url = nconf.get("url") + "/slack-broker/event/v1/toolchain/"
...
function registerPipelineWebhooks(req) {
	...
	request.get(options, function(error, response, body) {
		if (error) {
			logger.error(logPrefix + "Error while getting " + options.url + ":" + err);
...
function registerPipelineWebhook(req, pipeline) {
	...
	var webhook = {
		"label" : "temp webhook for pipeline " + pipeline.instance_id,
		"url" : nconf.get("url") + "/slack-broker/event/v1/unsecured/pipeline/" + pipeline.instance_id + "/event",
		"enabled" : true
	}
