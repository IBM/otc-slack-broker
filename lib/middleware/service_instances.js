...
function patchServiceInstance(req, res /*, next*/) {
    ...
    // TODO What can be patched for Slack ?
    // => API key, Channel id, Channel name, Channel Topic ?
    // If Channel (id or name) changes, then label must be changed  
    if (dashboardUrl) {
        patchParameters.dashboard_url = dashboardUrl;
    }

    if (parametersData) {
        patchParameters.parameters = parametersData;
    }
    ...
    db.get(serviceInstanceId, null, function(err, body) {
        ...
        if(!isValidOrganization(body.organization_guid, req.user.organizations)) {
            return res.status(403).json({ "description": "Error: User is not part of the organization." });
        }
        
        if (serviceId !== body.service_id) { 
            logger.error(logPrefix + "service_id " + serviceId + " did not match the service instance's service_id " + body.service_id);
            return res.status(400).json({ "description": "Error: service_id did not match the service instance's service_id." });
        }
        ...
        .existingDoc(null)
        .shouldCreate(false)
        .newDoc(_.extend(body, patchParameters))
        .shouldUpdate(function (published, proposed) {
            return published.parameters !== proposed.parameters ||
                   published.dashboard_url !== proposed.dashboard_url;
        })
        .update(function (err) {
                if (err) {
