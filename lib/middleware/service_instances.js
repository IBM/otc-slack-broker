...
function createOrUpdateServiceInstance (req, res) {
	...
	var db = req.servicesDb,
		...
		hasLabel = parametersData && parametersData.label;
	...
	db.get(serviceInstanceId, null, function(err, body) {
		if(err && err.statusCode !== 404) {
			...
			**/
			// TODO !!!
			return doServiceUpdate(res, req, db, serviceInstanceId, parametersData, "slack instance id", organizationId);
		} else {
			...
			**/
			return doServiceUpdate(res, req, db, serviceInstanceId, parametersData, instanceId, organizationId);
		}
	});
}
...
**/
function doServiceUpdate (res, req, db, serviceInstanceId, parametersData, instanceId, organizationId) {
		...
		.newDoc(_.extend(
			{
				...
				dashboard_url: nconf.get("services:slack") + instanceId,
