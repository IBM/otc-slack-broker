...
function createOrUpdateServiceInstance (req, res) {
	...
	if(req.user && !isValidOrganization(organizationId, req.user.organizations)) {
...
function bindServiceInstanceToToolchain (req, res/*, next*/) {
	...
	.shouldUpdate(function (published) {
		isOrg = !req.user || isValidOrganization(published.organization_guid, req.user.organizations);
