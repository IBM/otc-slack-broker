...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, parametersData) {
	...
	var parameters = {};
	if (api_token) {
		slackUtils.getSlackUser(nconf.get("services:slack-api"), api_token, function (err, user) {
			if (err) {
				return res.status(400).json({ "description": "Error: Slack API key not valid." });
			}
			parameters.api_token = api_token;
			slackUtils.getOrCreateSlackChannel(nconf.get("services:slack-api"), api_token, user, channel_id, channel_name, channel_topic, function (err, channel) {
				if (err) {
