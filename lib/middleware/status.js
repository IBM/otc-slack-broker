		...
		slack: function(callback) {
			...
			request.get(options, function(error, response, body) {
				var result = {
					type: "SERVICE",
					label: "Slack API",
					duration: elapsedTimeInMs(start)
				}
				if (error) {
					...
					result.timestamp = getTimestamp(response.headers['date']);
					if (body.ok) {
						result.status = "PASS";
...
function elapsedTimeInMs(startTime) {
	var diff = process.hrtime(startTime);
	return Math.floor((diff[0] * 1000) + (diff[1]/1000000));
}