...
function getPipelineInfo(pipelineId, toolchainCredentials, callback) {	
	...
	tiamUtil.getCredentials(toolchainCredentials, {"target": pipelineId}, function(err, credentials) {
		if (err) {
			logger.error(logPrefix + "No credentials obtained from TIAM : " + err);
			return callback(null, {label: pipelineId});
		}
		return getPipelineInfo_(pipelineId, toolchainCredentials, function(err, pipelineInfo) {
			if (err && err != 401) {
				return callback(null, {label: pipelineId});
			} if (err && err === 401) {
