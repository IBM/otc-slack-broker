...
function getStringForInput(dashboard_url, event) {
	var inputString = "";
	
	// com.ibm.team.integration.pipeline.toolchain.ToolchainUtils#getInputString
	
	if (!_.isEmpty(event.execution.inputs)) {
    	// First job in the stage provide the builderId used to compute the Build URL
		// Then retrieve the job whose id == builderId to find name
		var jobProvidingInput;
		if (!_.isEmpty(event.stage.jobs)) {
			var builderId = event.stage.jobs[0].builderId;
			if (builderId) {
				jobProvidingInput = _.findWhere(event.stage.jobs, {"id": builderId});;
			}
			//logger.info(logPrefix + "builderId:" + builderId);
		}
		//logger.info(logPrefix + "jobProvidingInput:" + jobProvidingInput);
		
		if (jobProvidingInput) {
	        _.map(event.execution.inputs, function(input) {
	        	// TODO For now, only consider that base10 numeric are valid Build identifier as SCM input are not resolved
	        	var inputUrl = dashboard_url + stage.id + "/" + builderId + "/" + inputId + "/artifacts";
	    		
	        	inputString += "<";
	        	inputString += inputUrl;
	        	inputString += "|";
	        	inputString += jobProvidingInput.componentName
	        	inputString += ">";
...
function getRequestor(requestedBy, toolchainCredentials, callback) {
	if (requestedBy) {
		if (requestedBy == null || requestedBy == "") {
			callback(null, "pipeline");
		} else {
			tiamUtil.getUserName(requestedBy, toolchainCredentials, callback);
		}
	} else {
		callback(null, "pipeline");
	}
...
function createMessageForStageStarted(event, toolchainCredentials, callback) {
	...
	async.parallel({
		...
		requestedBy: function(asyncCallback) {
			getRequestor(event.execution.requestedBy, toolchainCredentials, asyncCallback);
		}
		...
		if (err) {
			...
			var inputString = getStringForInput(results.pipelineInfo.dashboard_url, event);
