	...
	if (serviceInstanceId) {
		...
		db.get(serviceInstanceId, null, function(err, body) {
			if(err && err.statusCode !== 404) {
				...
				logger.debug(logPrefix + "Event from " + source + " in toolchain " + toolchainId + " for " + serviceInstanceId + ": About to be processed.");
				
				translator(payload, authorization, function (error, message) {
					if (error) {
						...
						logger.debug(logPrefix + "Event from " + source + " in toolchain " + toolchainId + " for " + serviceInstanceId + ": Posting Slack Message to channel " + channelName);
						...
						slackClient.postMessage(api_token, message, function(err, response) {
							if (err) {
								...
								logger.debug(logPrefix + "Event from " + source + " in toolchain " + toolchainId + " for " + serviceInstanceId + ": Slack Message to channel " + channelName + " sent");
								res.status(204).json({});
								return;
							}
						});			
...
function translate(event, authorization, callback) {
	var message = {};
	message.username = "Unknow";
