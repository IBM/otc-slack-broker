	...
	if (serviceInstanceId) {
		...
		db.get(serviceInstanceId, null, function(err, body) {
			if(err && err.statusCode !== 404) {
				...
				logger.debug(logPrefix + "[" + requestId + "]Event about to be processed - from " + source + " in toolchain " + toolchainId + " for " + serviceInstanceId);
				...
				translator(requestId, payload, authorization, function (error, message) {
					if (error) {
						...
						logger.debug(logPrefix + "[" + requestId + "]Posting Slack Message to channel " + channelName + "for Event - from " + source + " in toolchain " + toolchainId + " for " + serviceInstanceId);
						...
						slackClient.postMessage(api_token, message, function(err, response) {
							if (err) {
								...
								logger.debug(logPrefix + "[" + requestId + "]Slack Message to channel " + channelName + " sent for Event - from " + source + " in toolchain " + toolchainId + " for " + serviceInstanceId);
								res.status(204).json({});
								return;
							}
						});			
