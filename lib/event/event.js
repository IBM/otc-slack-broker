...
function incomingToolchainLifecycleEvent(req, res, next) {
	...
	if (process.env.NODE_ENV == "local-dev" || process.env.NODE_ENV == "dev-test") {
		...
		logger.debug(logPrefix + "Incoming ToolchainLifecycleEvent: " + JSON.stringify(incomingEvent.payload));
...
function incomingEventFromMessageStore(req, res, next) {
	...
	logger.debug(logPrefix + "Incoming event from Message store: " + JSON.stringify(incomingEvent.payload));
...
function processEvent(req, res, next) {
	...
	if (!translator) {
		...
		translator(requestId, payload, toolchainCredentials, function (error, message) {
			if (error) {
				return res.status(500).json({ "description" : error});
			} 
			// Find the api_token
			tiamUtil.getServiceData(serviceInstance.service_credentials, function(err, data) {
				...
				if (err == 404) {
					...
					logger.error(logPrefix + "No API key from TIAM Service - using parameters.api_token if any");
					api_token = serviceInstance.parameters.api_token;
					if (!api_token) {
						logger.error(logPrefix + "Error while retrieving API key from TIAM Service:" + err + ". No api key found.");
						return res.status(500).json({ "description" : "Error while retrieving API key"});					
					}
				} else if (err) {
					logger.error(logPrefix + "Error while retrieving API key from TIAM Service:" + err);
					return res.status(500).json({ "description" : "Error while retrieving API key"});					
				} else {
